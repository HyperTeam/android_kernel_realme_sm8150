...
bool __init tp_judge_ic_match(char *tp_ic_name)
{
	...
	switch(get_project()) {
		...
		case 19753:
		case 19700:																							  
		if (strstr(tp_ic_name, "novatek,nf_nt36672c") && (strstr(saved_command_line, "dsi_oppo19696boe_nt36672c_1080_2400_90fps_vid"))) {
				g_tp_dev_vendor = TP_BOE;
				tp_used_index = nt36672c;
				return true;
			}else if(strstr(tp_ic_name, "novatek,nf_nt36672c") && (strstr(saved_command_line, "dsi_oppo19696jdi_nt36672c_1080_2400_90fps_vid"))){
				g_tp_dev_vendor = TP_JDI;
				tp_used_index = nt36672c;
				return true;
			}
			else if(strstr(tp_ic_name, "himax,hx83112a_nf") && (strstr(saved_command_line, "mdss_dsi_oppo19696dsjm_hx83112a_1080_2340_vid"))) {
		...
		default:
			...
			//g_tp_dev_vendor = TP_JDI; //liuchao modify
			//tp_used_index = nt36672c;
			break;
	}
...
int tp_util_get_vendor(struct hw_resource *hw_res, struct panel_info *panel_data)
{
	...
	if((get_project()==19781)||(get_project()==19688)||(get_project()==19081)){
	prj_id = 19081;
	...
	snprintf(panel_data->fw_name, MAX_FW_NAME_LENGTH,
			"tp/%d/FW_%s_%s.img",prj_id, panel_data->chip_name, vendor);	
			//get_project(), panel_data->chip_name, vendor);

	if (panel_data->test_limit_name) {
		snprintf(panel_data->test_limit_name, MAX_LIMIT_DATA_LENGTH,
			"tp/%d/LIMIT_%s_%s.img",prj_id, panel_data->chip_name, vendor);
			//get_project(), panel_data->chip_name, vendor);
	}
