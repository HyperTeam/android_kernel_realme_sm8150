...
{
#define SAMPLE_COUNT_IN_OIS 7
#define SAMPLE_INTERVAL     4000
	int32_t i = 0;
	uint32_t *data = NULL;
	uint32_t kfifo_in_len = 0;
	uint32_t fifo_size_in_ois = SAMPLE_COUNT_IN_OIS*OIS_HALL_SAMPLE_BYTE;
	uint32_t fifo_size_in_ois_driver = OIS_HALL_SAMPLE_COUNT*OIS_HALL_SAMPLE_BYTE;
	struct timeval systemTime;
	uint64_t timestamp;
	struct cam_ois_ctrl_t *o_ctrl = (struct cam_ois_ctrl_t *)arg;
	uint32_t ois_hall_registers[SAMPLE_COUNT_IN_OIS] = {0x89C4, 0x89C0, 0x89BC, 0x89B8, 0x89B4, 0x89B0, 0x89AC};
	...
	while(1) {
		...
		for (i = SAMPLE_COUNT_IN_OIS - 1; i >= 0; i--) {
		...
		usleep_range(SAMPLE_COUNT_IN_OIS*SAMPLE_INTERVAL-5, SAMPLE_COUNT_IN_OIS*SAMPLE_INTERVAL);
	}
