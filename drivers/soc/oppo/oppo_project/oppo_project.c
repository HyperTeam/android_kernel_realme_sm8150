...
static char nfc_project[] = {5,8,32,33,34};
...
static int __init update_feature(void)
{
    ...
    mm_segment_t fs;
    fs = get_fs();
    projectName = get_Operator_Version();
    pr_err("update_feature, projectName [%d]", projectName);
    for (i=0; i<sizeof(nfc_project); i++){
        if(nfc_project[i] == projectName){
            isnfcproject = true;
        }
    }
