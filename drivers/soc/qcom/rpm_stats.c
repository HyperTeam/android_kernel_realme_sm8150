...
#define RPMSTATS_NUM 3
#define RPMSTATS_VDD_MIN_NAME   "aosd"
#define RPMSTATS_CX_MIN_NAME    "cxsd"
static struct msm_rpmstats_private_data *gPrvdata;
static bool init_flag = true;
static bool force_format = false;
struct rpmstats_cnt_data {
	u32 last_count;
	u32 same_times;
};
static struct rpmstats_cnt_data rpmstats_cnt_data_array[RPMSTATS_NUM];
#endif
/*VENDOR_EDIT*/
...
//yangmingjin@BSP.POWER.Basic 2019/05/30 addfor RM_TAG_POWER_DEBUG
	...
	u32 last_count;
	u32 same_times;
	int state_index = RPMSTATS_NUM - 1;
#endif
/*VENDOR_EDIT*/
...
//yangmingjin@BSP.POWER.Basic 2019/05/30 add for RM_TAG_POWER_DEBUG
#ifdef VENDOR_EDIT
	if(0 == strncmp(stat_type, RPMSTATS_VDD_MIN_NAME, sizeof(RPMSTATS_VDD_MIN_NAME)-1)){
		state_index = 0;
	}else if(0 == strncmp(stat_type, RPMSTATS_CX_MIN_NAME, sizeof(RPMSTATS_CX_MIN_NAME)-1)){
		state_index = 1;
	}else{}

    if(rpmstats_cnt_data_array[state_index].last_count == data->count)
        rpmstats_cnt_data_array[state_index].same_times++;
    else
        rpmstats_cnt_data_array[state_index].same_times = 0;
    last_count = rpmstats_cnt_data_array[state_index].last_count;
    same_times = rpmstats_cnt_data_array[state_index].same_times;
    rpmstats_cnt_data_array[state_index].last_count = data->count;

	if(force_format)
#if defined(CONFIG_MSM_RPM_SMD)
		return snprintf(buf, buflength, "%s[%d %d %d %llu %#010x] ",
			stat_type, data->count, last_count, same_times, actual_last_sleep, data->client_votes);
#else
	return snprintf(buf, buflength, "%s[%d %d %d %llu] ",
		stat_type, data->count, last_count, same_times, actual_last_sleep);
#endif
	else
#endif
/*VENDOR_EDIT*/
...
static bool is_over_xo_thres(void){
	return rpmstats_cnt_data_array[1].same_times > RPMSTATS_OVER_CNT_THRES;
}
static bool is_over_aosd_thres(void){
	return rpmstats_cnt_data_array[0].same_times > RPMSTATS_OVER_CNT_THRES;
}
...
static void rpmstats_print_format(struct msm_rpmstats_private_data *prvdata, bool suspend){
	...
	if (prvdata->read_idx < prvdata->num_records)
		prvdata->len = msm_rpmstats_copy_stats(prvdata);
#if defined(CONFIG_MSM_RPM_SMD)
	printk(KERN_INFO"[RM_POWER]rpmstats_%s:{mode[cnt pre_cnt same_times totalMs client_vot]} %s}\n",
		(suspend==true)?"S":"R", prvdata->buf);
#else
	printk(KERN_INFO"[RM_POWER]rpmstats_%s:{mode[cnt pre_cnt same_times totalMs]} %s}\n",
		(suspend==true)?"S":"R", prvdata->buf);
#endif
}
